...
Data: 2026-08-03
Stato: accettata
Contesto: nel ciclo di lavoro fra il 2026-07-29 e il 2026-08-03 sono entrate sette fonti nuove, alcune
lette per intero, alcune di cui si e letto solo l'abstract, alcune note soltanto attraverso la
citazione di un'altra fonte. Un errore concreto e stato commesso proprio su questo: la parafrasi di
Butler contenuta in un articolo terzo e stata riportata come se fossero parole di Butler, e la
distinzione fra "letto" e "riferito da altri" viveva soltanto nella prosa delle note, dove si perde.
Decisione: il grado di verifica di una fonte e un dato di prima classe e si dichiara su quattro
gradini. Contenuto letto per intero, e allora si puo citare il contenuto con virgolette e pagina.
Contenuto letto in parte, e allora si cita quella porzione dichiarando cosa non si e letto. Anagrafica
verificata e contenuto non letto, e allora si puo scrivere la voce bibliografica ma non citare il
contenuto. Esistenza soltanto segnalata, e allora non si scrive nulla nel `.bib` e la fonte resta
candidata. Il livello si scrive in maiuscolo all'inizio del campo `note` di ogni voce `.bib`, cosi da
essere visibile a colpo d'occhio rileggendo il file, e si registra in `bib_verified_by` nel registro.
Motivazione: una voce `.bib` asserisce l'anagrafica, non il contenuto, e le due cose si verificano
separatamente e con fatica diversa. Tenerle distinte impedisce la classe di errori in cui si attribuisce
a un autore cio che un altro ha riassunto di lui. La procedura completa e codificata nella skill
`fonte-nuova`.
Conseguenze: `book-bib-extract` resta valida per il caso che copre, cioe il libro ingerito di cui si
legge il colophon, e `fonte-nuova` copre il caso piu frequente, cioe articoli, capitoli, paper dietro
paywall, link e video. Una fonte contestata in letteratura si registra insieme alla sua replica, e nel
campo `note` di entrambe si scrive che vanno citate insieme: e il caso di Butler 1989 e Krumhansl 1990.
Nessuna fonte sale a `verificata` sulla base di una sintesi di ricerca invece della lettura.

## ADR-009 — Strumenti che implementano il contenuto del libro, e primato delle definizioni del libro sugli strumenti

Data: 2026-08-03
Stato: accettata
Contesto: durante la stesura del capitolo sul tritono sono nati quattro strumenti Python che non
documentano il libro ma ne implementano le affermazioni: `tritoni-scale.py` calcola il contenuto di
tritoni delle scale, `derivazione-scale.py` implementa la macchina delle dominanti secondarie ai due
livelli, `skill-freshness.py` rileva la deriva fra una skill e le sue fonti, `probe-pdf-text.py`
misura la qualita del testo estraibile di un corpus. Due di essi hanno cambiato il contenuto del libro
invece di limitarsi a confermarlo: il primo ha smentito l'intuizione sulla corrispondenza biunivoca
fra scale e coppie di tritoni, il secondo ha fatto emergere la scala napoletana minore, che era
registrata come direzione futura sospesa per mancanza di fonti.
Decisione: gli strumenti che implementano affermazioni del libro sono parte del metodo del progetto e
si versionano, e per essi vale una regola vincolante: le definizioni operative implementate negli
strumenti sono quelle del libro, non quelle scolastiche. In concreto, per la macchina delle
derivazioni un accordo e una dominante quando possiede il tritono fra terza maggiore e settima minore,
indipendentemente dalla quinta.
Motivazione: la regola nasce da un errore reale. L'agente aveva richiesto la quinta giusta per
trasformare il semidiminuito del settimo grado in dominante, contando due alterazioni invece di una,
e su questa base aveva scritto nel capitolo che quel grado non rispetta la doppia eredita. L'errore e
stato rilevato dall'utente. Con la definizione del libro la derivazione costa una alterazione sola e
produce la napoletana minore. Un'implementazione che usi la definizione scolastica contraddice la
tesi che dovrebbe verificare, e per giunta e esattamente l'errore che il capitolo rimprovera ad altri.
Conseguenze: prima di usare uno strumento per verificare un'affermazione del libro si controlla che le
sue definizioni siano quelle del libro. Un risultato controintuitivo prodotto da uno strumento va
trattato come possibile errore dell'implementazione prima che come scoperta. E un risultato che
contraddice il libro va portato all'utente con il calcolo in chiaro, non applicato al testo
direttamente: e cosi che l'errore sul settimo grado e stato trovato.
